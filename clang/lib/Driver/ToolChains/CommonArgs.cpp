...
tools::ParsePICArgs(const ToolChain &ToolChain, const ArgList &Args) {
  ...
  Arg* LastSINGLE_PIC_BASEArg = Args.getLastArg(options::OPT_fsingle_pic_base, options::OPT_fno_single_pic_base);
  if (LastSINGLE_PIC_BASEArg && LastSINGLE_PIC_BASEArg->getOption().matches(options::OPT_fsingle_pic_base)) {
    if (!EmbeddedPISupported)
        ToolChain.getDriver().Diag(diag::err_drv_unsupported_opt_for_target)
            << LastRWPIArg->getSpelling() << Triple.str();
